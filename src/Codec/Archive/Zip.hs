...
import qualified Data.ByteString.Lazy.Internal as BB
...
data CentralDirectoryEnd = CentralDirectoryEnd 
                             { cdeCentralDirectorySize :: Word32
                             , cdeTotal                :: Word16
                             , cdeOffset               :: Word32
                             } deriving (Show)

szCentralDirectoryEnd :: (Num a) => a
szCentralDirectoryEnd = 4 + 2 + 2 + 2 + 2 + 4 + 4 + 2

findEnd :: Handle -> IO CentralDirectoryEnd
findEnd h = allocaArray szCentralDirectoryEnd $ \p -> do
    hSeek h SeekFromEnd (-szCentralDirectoryEnd)
    pos <- hTell h
    findEndRec h p pos 0
    where 
        findEndRec :: Handle -> Ptr Word8 -> Integer -> Int -> IO CentralDirectoryEnd
        findEndRec h p !pos !left | pos > 0 = do
            hGetBuf h p szCentralDirectoryEnd
            ced <- getCentralDirectoryEnd left p
            case ced of
                Just ced -> return ced
...
getCentralDirectoryEnd :: Int -> Ptr Word8 -> IO (Maybe CentralDirectoryEnd)
getCentralDirectoryEnd !left !p = do  
  sig <- word32le p 0  
  if fromIntegral sig /= 0x06054b50 
    then return Nothing
    else do -- skip 2 -- number of this disk
            -- skip 2 -- number of the disk with the start of the central directory
            -- skip 2 -- total number of entries in the central directory on this disk
            total <- word16le p (4 + 2 + 2 + 2)
            sz    <- word32le p (4 + 2 + 2 + 2 + 2)
            offs  <- word32le p (4 + 2 + 2 + 2 + 2 + 4)
            commentLength <- word16le p (4 + 2 + 2 + 2 + 2 + 4 + 4)
            if left == fromIntegral commentLength
              then return $ Just $ CentralDirectoryEnd
                                   { cdeCentralDirectorySize = sz 
                                   , cdeTotal                = total
                                   , cdeOffset               = offs
                                   }
              else return Nothing
  where 
      word32le p offs = do b1 :: Word32 <- fromIntegral `liftM` peekElemOff p offs
                           b2 :: Word32 <- fromIntegral `liftM` peekElemOff p (offs+1)
                           b3 :: Word32 <- fromIntegral `liftM` peekElemOff p (offs+2)
                           b4 :: Word32 <- fromIntegral `liftM` peekElemOff p (offs+3)
                           return $ (b4 `shiftL` 24) .|. (b3 `shiftL` 16) .|. (b2 `shiftL` 8) .|. b1
      word16le p offs = do b1 :: Word16 <- fromIntegral `liftM` peekElemOff p offs
                           b2 :: Word16 <- fromIntegral `liftM` peekElemOff p (offs+1)                           
                           return $ (b2 `shiftL` 8) .|. b1
---------------------------------------------------------------------------------
...
setFileTimeStamp file epochtime = do return ()  -- TODO - figure out how to set the timestamp on Windows
...
getArchive :: B.ByteString -- ^ archive body
           -> Get Archive
getArchive body = do  
  files <- many (getFileHeader body)
...
getLocalFile :: Word32            -- ^ compressed size of the entry
             -> Get B.ByteString
getLocalFile compressedSize = do
  sig <- lookAhead getWord32le
  if sig /= 0x04034b50
    then fail "expected local file header signature"
    else do      
      skip 4  -- signature
      skip 2  -- version
...
getFileHeader :: B.ByteString     -- ^ archive body
              -> Get (Maybe Entry)
getFileHeader body = do
  sig <- lookAhead getWord32le
  if sig /= 0x02014b50
     then return Nothing
     else do
       ...
       let compressedData = runGet (getLocalFile compressedSize) (B.drop (fromIntegral relativeOffset) body)
       return $ Just $ Entry
